...
//	private MemberService memberService;
	 ...
	 * String login(MemberDTO member, HttpServletRequest request, HttpSession
	 ...
	 * find_id);
	 * 
	 * String member_id = member.getMember_id(); MemberDTO getMember =
	 * memberService.readMember(member_id);
	 * 
	 ...
	 * 
	 * 
	 * if (!member.getMember_id().equals(getMember.getMember_id()) ||
	 * !member.getMember_pw().equals(getMember.getMember_pw())) {
	 * 
	 * request.setAttribute("msg", "아이디 또는 비밀번호를 확인하세요");
	 * request.setAttribute("url", "login.do"); return "alert";
	 * 
	 * } else {
	 * 
	 ...
	 * session.setAttribute("MemberRoadAddress",
	 * getMember.getMember_road_address());
	 * session.setAttribute("MemberJibunAddress",
	 * getMember.getMember_jibun_address());
	 * session.setAttribute("MemberDetailAddress",
	 * getMember.getMember_detail_address()); return "redirect:/"; // viewResolver를
	 * 이용하면 WEB-INF로 경로 설정되기 때문에 바로 이동시킨다.
	 * 
	 * //}
	 * 
	 * } else { return "all/cafehi_login"; } }
	 */
